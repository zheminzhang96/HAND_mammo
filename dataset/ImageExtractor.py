...
def extract_images(filedata, i, png_destination, flattened_to_level, failed, is16Bit):
    ...
    try:
        ...
        if is16Bit:
            ...
            image_2d= dicom.pixel_data_handlers.apply_voi_lut(image_2d,ds).astype(np.double)
            # # Rescaling grey scale between 0-65535
...
def extraction_wrapper(i=None,filedata=None,png_destination=None,flattened_to_level=None,failed=None,is16bit=None): 
    if   filedata is None or i is None or png_destination is None or flattened_to_level is None or failed is None or is16bit is None: 
        raise Exception("messed up ")
    if (filedata.iloc[i].loc['file'] is not np.nan):
        extract_outs  = extract_images(filedata, i, png_destination, flattened_to_level, failed,is16bit) 
    else:  
        extract_outs = None 
    return extract_outs
...
def execute(pickle_file, dicom_home, output_directory, print_images, print_only_common_headers, depth,
    ...
    for i, chunk in enumerate(file_chunks):
        ...
        if print_images:
            ...
            total = len(chunk)
            stamp = time.time() 
            proc_data = partial(extraction_wrapper,filedata=filedata,png_destination=png_destination,flattened_to_level=flattened_to_level,failed=failed,is16bit=is16Bit)
            with Pool(30) as p: 
                for out in tqdm(p.imap_unordered(proc_data,range(len(filedata))),total=len(filedata)) :  
                    if not out: 
                        continue
