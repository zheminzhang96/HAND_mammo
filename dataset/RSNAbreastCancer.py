...
def get_breast_data(laterality):
    rootdir = '/home/zhemin/Datacenter_storage/RSNA_mammo/train_images_png/'
    ...
    df_org = pd.read_csv("/home/zhemin/Datacenter_storage/RSNA_mammo/train.csv", dtype='str')
    ...
    return train_df_cc, test_df_cc


def check_laterality(x, laterality):
    correct_laterality = True
    if laterality == 'R':
        if np.mean(x[:,0:40]) > np.mean(x[:,-40:-1]):  # x[:,0] is the far left
            laterality = 'L'
            correct_laterality = False

    elif laterality == 'L':
        if np.mean(x[:,0:40]) < np.mean(x[:,-40:-1]):
            laterality = 'R'
            correct_laterality = False
